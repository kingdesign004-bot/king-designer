...
import { BadgeCheck, Copy, Crown, Flag, Loader2, MapPin, MessageCircle, Share2, ShieldBan, Sparkles, UserPlus, Briefcase, Wrench, Link2, Calendar, CheckCircle, Star } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const [editing, setEditing] = useState(false); const [showReport, setShowReport] = useState(false);
  ...
  const [clientCompany, setClientCompany] = useState(""); const [clientBudget, setClientBudget] = useState(""); const [clientNeeds, setClientNeeds] = useState("");
  const ratings = trpc.ratings.list.useQuery({ designerId: userId }, { enabled: Number.isFinite(userId) && profile?.accountType === "designer" });
  const rate = trpc.ratings.create.useMutation({ onSuccess: () => { toast.success("تم حفظ تقييمك"); utils.ratings.list.invalidate({ designerId: userId }); }, onError: e => toast.error(e.message) });

  useEffect(() => { if (profile && current?.id === profile.id && location.includes("edit=1")) beginEdit(); }, [profile?.id, current?.id, location]);
  ...
  const isPrestige = Boolean(profile.verified) || isAdmin || profile.plan === "vip";
  ...
  const beginEdit = () => { setName(profile.name || ""); setHandle(profile.handle || ""); setBio(profile.bio || ""); setSpecialty(profile.specialty || ""); setCountry(countryLabel || ""); setLevel(levelLabel); setAvatarFile(null); setCoverFile(null); setAvatarPreview(profile.avatarUrl || ""); setCoverPreview(profile.coverUrl || ""); setExperience((profile as any).experience || ""); setSkills((profile as any).skills || ""); setTools((profile as any).tools || ""); setPortfolioUrl((profile as any).portfolioUrl || ""); setYearsOfExperience((profile as any).yearsOfExperience || ""); setAvailability((profile as any).availability || ""); setClientCompany((profile as any).clientCompany || ""); setClientBudget((profile as any).clientBudget || ""); setClientNeeds((profile as any).clientNeeds || ""); setEditing(true); };
  ...
  const saveProfile = async () => {
    try {
      ...
      await update.mutateAsync({ name, handle: handle || undefined, bio, specialty, country, avatarUrl: avatar?.url, coverUrl: cover?.url, experience, skills, tools, portfolioUrl, yearsOfExperience, availability, clientCompany, clientBudget, clientNeeds });
    ...
    <div className="container py-8">
      <section className={`overflow-hidden rounded-[2rem] border bg-white shadow-sm ${isPrestige ? "border-[#d7ab39] ring-1 ring-[#f6c94c]/30" : "border-neutral-200"}`}>
        <div className="relative h-56 overflow-hidden bg-neutral-950">
          ...
          <div className={`absolute bottom-4 right-5 inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold backdrop-blur ${isAdmin ? "border-[#f6c94c] bg-[#221806]/90 text-[#f6c94c]" : "border-[#f6c94c]/40 bg-neutral-950/80 text-[#f6c94c]"}`}><Crown className="h-4 w-4" />{isAdmin ? "إدارة موثّقة" : isPrestige ? "ملف مميز" : "ملف إبداعي"}</div>
        </div>
        <div className="relative px-6 pb-7">
          <div className="-mt-12 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex flex-1 items-end gap-4">
              <div className={`grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-[1.5rem] border-4 border-white text-2xl font-bold text-[#f6c94c] shadow-xl ${isAdmin ? "bg-[radial-gradient(circle_at_30%_20%,#f6c94c,#221806_55%,#070707)] ring-4 ring-[#f6c94c]/30" : "bg-neutral-900"}`}>{profile.avatarUrl ? <img src={profile.avatarUrl} alt={profile.name || "المستخدم"} className="h-full w-full object-cover" /> : (profile.name || "KD").slice(0, 2)}</div>
              <div className="min-w-0 flex-1 pt-14 sm:pt-12">
                <div className="flex flex-wrap items-center gap-2"><h1 className={`font-display text-2xl font-extrabold tracking-tight ${isPrestige ? "bg-gradient-to-l from-[#8b5a00] via-[#f6c94c] to-[#9d6b0a] bg-clip-text text-transparent" : "text-neutral-950"}`}><ColoredName user={profile} /></h1>{(profile.verified || isAdmin) ? <BadgeCheck className="h-5 w-5 fill-[#f6c94c] text-neutral-950" aria-label={isAdmin ? "إدارة موثقة" : "حساب موثق"} /> : null}{isAdmin && <span className="rounded-full bg-[#221806] px-2 py-1 text-[10px] font-bold text-[#f6c94c]">ADMIN</span>}</div>
            ...
            <div className="flex flex-wrap gap-2">{isMine ? <Button onClick={beginEdit} className="rounded-xl bg-neutral-950 text-white">تعديل الملف</Button> : <>{!isAdmin && isAuthenticated && <Button onClick={() => incomingStatus === "pending" ? respond.mutate({ followerId: profile.id, accept: true }) : followStatus === "accepted" ? toggleFollow.mutate({ userId: profile.id }) : followStatus === "pending" ? undefined : follow.mutate({ userId: profile.id })} disabled={follow.isPending || toggleFollow.isPending || respond.isPending || followStatus === "pending"} className="rounded-xl bg-neutral-950 text-white"><UserPlus className="ml-2 h-4 w-4" />{incomingStatus === "pending" ? "قبول المتابعة" : followStatus === "accepted" ? "إلغاء المتابعة" : followStatus === "pending" ? "قيد الانتظار" : "متابعة"}</Button>}<Button onClick={() => conversation.mutate({ userId: profile.id })} disabled={!isAuthenticated || conversation.isPending} variant="outline" className="rounded-xl"><MessageCircle className="ml-2 h-4 w-4" />{isAdmin ? "تواصل مع الإدارة" : "رسالة"}</Button>{!isAdmin && <><Button onClick={() => block.mutate({ userId: profile.id })} variant="outline" className="rounded-xl text-red-600"><ShieldBan className="ml-2 h-4 w-4" />حظر</Button><Button onClick={() => { if (!isAuthenticated) return toast.error("سجّل الدخول للإبلاغ"); setShowReport(true); }} variant="outline" className="rounded-xl"><Flag className="ml-2 h-4 w-4" />إبلاغ</Button></>}</>}</div>
          </div>
          <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_330px]">
            <div>
              <p className="max-w-2xl text-sm leading-8 text-neutral-600">{profile.bio || "لم يضف هذا المستخدم نبذة بعد."}</p>
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-neutral-500">{countryLabel && <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{countryLabel}</span>}<span className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 font-bold ${isPrestige ? "bg-gradient-to-l from-[#fff0ae] to-[#fff8df] text-[#8b5a00]" : "bg-[#f8f7f3] text-neutral-600"}`}><Crown className="h-3.5 w-3.5" />المستوى {levelLabel}</span></div>
              {(profile as any).experience || (profile as any).skills || (profile as any).tools || (profile as any).portfolioUrl || (profile as any).yearsOfExperience || (profile as any).availability ? (
                <div className="mt-6 rounded-2xl border border-neutral-200 bg-[#f8f7f3] p-5">
                  <h3 className="mb-3 flex items-center gap-2 font-display text-sm font-bold text-neutral-800"><Briefcase className="h-4 w-4 text-[#c38a10]" />السيرة الذاتية</h3>
                  <div className="space-y-3 text-sm">
                    {(profile as any).yearsOfExperience && <div className="flex items-center gap-2 text-neutral-600"><Calendar className="h-4 w-4 text-neutral-400" />سنوات الخبرة: {(profile as any).yearsOfExperience}</div>}
                    {(profile as any).availability && <div className="flex items-center gap-2 text-neutral-600"><CheckCircle className="h-4 w-4 text-emerald-500" />الحالة: {(profile as any).availability}</div>}
                    {(profile as any).experience && <div className="text-neutral-600"><span className="font-bold">الخبرة:</span> {(profile as any).experience}</div>}
                    {(profile as any).skills && <div className="text-neutral-600"><span className="font-bold">المهارات:</span> {(profile as any).skills}</div>}
                    {(profile as any).tools && <div className="flex items-start gap-2 text-neutral-600"><Wrench className="mt-0.5 h-4 w-4 shrink-0 text-neutral-400" /><div><span className="font-bold">الأدوات:</span> {(profile as any).tools}</div></div>}
                    {(profile as any).portfolioUrl && <a href={(profile as any).portfolioUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#a36e00] hover:underline"><Link2 className="h-4 w-4" />معرض الأعمال</a>}
                  </div>
                </div>
              ) : null}
            </div>
      ...
      {editing && <div className="mt-5 rounded-[1.5rem] border border-[#ead48d] bg-[#fff8df] p-6"><h2 className="font-display font-bold">تعديل معلوماتك</h2><div className="mt-4 grid gap-3 md:grid-cols-2"><Input value={name} onChange={e => setName(e.target.value)} placeholder="الاسم" className="rounded-xl bg-white" /><Input value={handle} onChange={e => setHandle(e.target.value.replace(/[^A-Za-z0-9_\u0600-\u06FF]/g, ""))} placeholder="المعرّف الفريد (handle)" className="rounded-xl bg-white" /><Input value={specialty} onChange={e => setSpecialty(e.target.value)} placeholder="التخصص" className="rounded-xl bg-white" /><Input value={country} onChange={e => setCountry(e.target.value)} placeholder="الدولة — يحددها النظام أو الإدارة" disabled className="rounded-xl bg-neutral-100 text-neutral-500" /><Input value={level} readOnly placeholder="المستوى" className="rounded-xl bg-neutral-100 text-neutral-500" /><Textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="نبذة" className="rounded-xl bg-white md:col-span-2" /><Input value={yearsOfExperience} onChange={e => setYearsOfExperience(e.target.value)} placeholder="سنوات الخبرة (مثال: 5+ سنوات)" className="rounded-xl bg-white" /><Input value={availability} onChange={e => setAvailability(e.target.value)} placeholder="الحالة (مثال: متاح للعمل)" className="rounded-xl bg-white" />{profile.accountType === "client" && <><Input value={clientCompany} onChange={e => setClientCompany(e.target.value)} placeholder="الشركة أو نوع النشاط" className="rounded-xl bg-white" /><Input value={clientBudget} onChange={e => setClientBudget(e.target.value)} placeholder="الميزانية أو نطاقها" className="rounded-xl bg-white" /><Textarea value={clientNeeds} onChange={e => setClientNeeds(e.target.value)} placeholder="احتياجاتك من المصممين والمشروع" className="rounded-xl bg-white md:col-span-2" /></>}<Textarea value={experience} onChange={e => setExperience(e.target.value)} placeholder="الخبرة المهنية - اذكر أبرز المشاريع والشركات" className="rounded-xl bg-white md:col-span-2" /><Textarea value={skills} onChange={e => setSkills(e.target.value)} placeholder="المهارات (افصل بفاصلة: تصميم شعارات، هوية بصرية، ...)" className="rounded-xl bg-white md:col-span-2" /><Textarea value={tools} onChange={e => setTools(e.target.value)} placeholder="الأدوات (مثال: Photoshop, Illustrator, Figma)" className="rounded-xl bg-white md:col-span-2" /><Input value={portfolioUrl} onChange={e => setPortfolioUrl(e.target.value)} placeholder="رابط معرض الأعمال (Behance, Dribbble...)" className="rounded-xl bg-white md:col-span-2" /><div className="grid gap-3 sm:grid-cols-2"><label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-neutral-300 bg-white px-4 py-3 text-xs text-neutral-500"><span className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl bg-neutral-100">{avatarPreview ? <img src={avatarPreview} alt="معاينة الصورة الشخصية" className="h-full w-full object-cover" /> : "لا توجد"}</span><span>اختيار الصورة الشخصية<input type="file" accept="image/jpeg,image/webp,image/png" className="hidden" onChange={e => chooseImage(e.target.files?.[0], "avatar")} /></span></label><label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-neutral-300 bg-white px-4 py-3 text-xs text-neutral-500"><span className="grid h-14 w-20 shrink-0 items-center overflow-hidden rounded-xl bg-neutral-100">{coverPreview ? <img src={coverPreview} alt="معاينة صورة الغلاف" className="h-full w-full object-cover" /> : "لا يوجد"}</span><span>اختيار صورة الغلاف<input type="file" accept="image/jpeg,image/webp,image/png" className="hidden" onChange={e => chooseImage(e.target.files?.[0], "cover")} /></span></label></div></div><div className="mt-4 flex gap-2"><Button onClick={saveProfile} disabled={update.isPending || mediaUpload.isPending} className="rounded-xl bg-neutral-950 text-white">{update.isPending || mediaUpload.isPending ? <Loader2 className="animate-spin" /> : "حفظ التعديلات"}</Button><Button variant="ghost" onClick={() => setEditing(false)} className="rounded-xl">إلغاء</Button></div></div>}
      {isMine && !isAdmin && <AccountVerificationCard profile={profile} />}
      {profile.accountType === "designer" && <section className="mt-5 rounded-[1.5rem] border border-[#ead48d] bg-[#fff8df] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display font-bold">تقييمات العملاء</h2><p className="mt-1 text-xs text-neutral-500">{profile.ratingCount || 0} تقييم — متوسط {Number(profile.ratingAverage || 0).toFixed(1)} / 5</p></div><div className="flex items-center gap-1 text-[#c38a10]">{[1,2,3,4,5].map(star => <Star key={star} className={`h-4 w-4 ${star <= Math.round(Number(profile.ratingAverage || 0)) ? "fill-current" : ""}`} />)}</div></div>{isAuthenticated && current?.accountType === "client" && !isMine && <div className="mt-4 flex flex-wrap items-center gap-2"><select id="rating-score" className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm"><option value="5">5 نجوم</option><option value="4">4 نجوم</option><option value="3">3 نجوم</option><option value="2">نجمتان</option><option value="1">نجمة واحدة</option></select><Button onClick={() => { const score = Number((document.getElementById("rating-score") as HTMLSelectElement)?.value || 5); rate.mutate({ designerId: profile.id, score }); }} className="rounded-xl bg-neutral-950 text-white">أضف تقييمًا</Button></div>}{ratings.data?.slice(0, 3).map(({ rating, reviewer }: any) => <div key={rating.id} className="mt-3 rounded-xl bg-white p-3 text-sm"><div className="flex items-center justify-between"><span className="font-bold">{reviewer.name || "عميل"}</span><span className="text-[#c38a10]">{"★".repeat(rating.score)}</span></div>{rating.body && <p className="mt-1 text-neutral-600">{rating.body}</p>}</div>)}</section>}
  ...
  <ReportDialog open={showReport} targetType="user" targetId={profile.id} onClose={() => setShowReport(false)} onSuccess={() => {}} /></AppChrome>;
}
