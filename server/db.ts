...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  if (user.openId === ENV.ownerOpenId) {
    values.publicId = "10000";
    updateSet.publicId = "10000";
  } else if (!existing?.publicId) {
    let publicId = String(randomInt(10001, 999999));
    while ((await db.select({ id: users.id }).from(users).where(eq(users.publicId, publicId)).limit(1))[0])
      publicId = String(randomInt(10001, 999999));
    values.publicId = publicId;
    updateSet.publicId = publicId;
  }
  ...
  if (user.birthYear !== undefined) {
    values.birthYear = user.birthYear ?? null;
    updateSet.birthYear = user.birthYear ?? null;
  }
...
export async function getProfile(userId: number) {
  ...
  return {
    ...
    ratingAverage: Number(ratingSummary[0]?.average ?? 0),
    ratingCount: Number(ratingSummary[0]?.count ?? 0),
  };
}
...
export function filterSearchResults<TUser extends { id: number }, TPost extends { author: { id: number } }>(usersResult: TUser[], postsResult: TPost[], blockedIds: Set<number>) {
  return {
    users: filterBlockedRecords(usersResult, blockedIds, user => user.id),
    posts: filterBlockedRecords(postsResult, blockedIds, post => post.author.id),
  };
}
...
export async function listFeed(viewerId?: number) {
  ...
  return Promise.all(
    visibleRows.map(async ({ post, author }) => {
      ...
      if (viewerId)
        liked = Boolean(
          (await db.select({ id: postLikes.id }).from(postLikes).where(and(eq(postLikes.postId, post.id), eq(postLikes.userId, viewerId))).limit(1))[0]
        );
      let shared = false;
      if (viewerId)
        shared = Boolean(
          (await db.select({ id: postShares.id }).from(postShares).where(and(eq(postShares.postId, post.id), eq(postShares.userId, viewerId))).limit(1))[0]
        );
      const activeSubscription = (
        await db
          .select({ plan: subscriptions.plan })
          .from(subscriptions)
          .where(and(eq(subscriptions.userId, author.id), eq(subscriptions.status, "active"), or(isNull(subscriptions.endsAt), gt(subscriptions.endsAt, new Date()))))
          .limit(1)
      )[0];
      return {
        ...post,
        author: { ...author, plan: activeSubscription?.plan ?? "free" },
        media,
...
export async function createNotification(
  recipientId: number,
  actorId: number,
  type: "follow" | "like" | "comment" | "reply" | "share" | "message",
  postId?: number,
  commentId?: number,
  body?: string
) {
  const db = await getDb();
  if (!db || recipientId === actorId) return;
